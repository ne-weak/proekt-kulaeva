const questions = [
    { q: "Вы мгновенно реагируете на происходящее, часто не успев подумать?", type: "choleric" },
    { q: "Вам жизненно необходимо менять обстановку и круг общения?", type: "sanguine" },
    { q: "Вас практически невозможно вывести из себя?", type: "phlegmatic" },
    { q: "Вы замечаете тончайшие детали в поведении людей и остро на них реагируете?", type: "melancholic" },
    { q: "Ваша речь быстрая, эмоциональная и сопровождается активной мимикой?", type: "choleric" },
    { q: "Вы легко справляетесь с несколькими делами одновременно?", type: "sanguine" },
    { q: "Вы долго обдумываете свои слова, прежде чем высказаться?", type: "phlegmatic" },
    { q: "Мелкие неудачи способны надолго выбить вас из колеи?", type: "melancholic" },
    { q: "Вы склонны к риску и часто берете на себя роль лидера в конфликте?", type: "choleric" },
    { q: "Вы предпочитаете яркие впечатления глубокому анализу ситуации?", type: "sanguine" },
    { q: "В работе вам важнее всего порядок, предсказуемость и тишина?", type: "phlegmatic" },
    { q: "Вы часто чувствуете потребность в одиночестве для восстановления сил?", type: "melancholic" }
];

const results = {
    sanguine: { 
        title: "Сангвиник", 
        desc: "<b>Психологический портрет:</b> Живой, подвижный человек с частой сменой впечатлений. <br><br><b>В обществе:</b> Вы — настоящий «социальный магнит». В любой компании вы быстро становитесь центром внимания, легко гасите конфликты шутками и заряжаете других своим энтузиазмом. Вы идеальный коммуникатор, который умеет договариваться даже с самыми сложными людьми.",
        fact: "Интересный факт: Согласно исследованиям, сангвиники проще всего переносят смену часовых поясов и быстрее других типов адаптируются к новой среде."
    },
    choleric: { 
        title: "Холерик", 
        desc: "<b>Психологический портрет:</b> Быстрый, страстный, порывистый, однако совершенно неуравновешенный тип. <br><br><b>В обществе:</b> Вы — лидер-реформатор. В группе вы часто берете на себя ответственность в критические моменты. Ваша прямолинейность помогает делу, но иногда ранит окружающих. Вы тот, кто заставляет общество и проекты двигаться вперед вопреки преградам.",
        fact: "Интересный факт: Холерики обладают самой быстрой реакцией среди всех типов, что делает их отличными спринтерами и антикризисными менеджерами."
    },
    phlegmatic: { 
        title: "Флегматик", 
        desc: "<b>Психологический портрет:</b> Невозмутимый человек со спокойными стремлениями и ровным настроением. <br><br><b>В обществе:</b> Вы — «островок стабильности». В коллективе вас ценят за отсутствие интриг и умение объективно оценивать ситуацию без лишних эмоций. Вы лучший слушатель, к которому друзья приходят за по-настоящему дельным и взвешенным советом.",
        fact: "Интересный факт: Флегматики способны дольше всех сохранять высокую концентрацию при монотонной работе, где другие типы быстро начинают совершать ошибки."
    },
    melancholic: { 
        title: "Меланхолик", 
        desc: "<b>Психологический портрет:</b> Впечатлительный человек, склонный к глубоким переживаниям даже незначительных событий. <br><br><b>В обществе:</b> Вы — эмпат и глубокий аналитик. В отношениях вы самый преданный друг, который чувствует состояние другого человека без слов. В группе вы замечаете риски и тонкие детали, которые упускают остальные, обеспечивая глубину любому проекту.",
        fact: "Интересный факт: Статистически, большинство выдающихся творцов, поэтов и философов были меланхоликами благодаря их способности к глубочайшей рефлексии."
    }
};

let currentIdx = 0;
let scores = { sanguine: 0, choleric: 0, phlegmatic: 0, melancholic: 0 };

function handleAnswer(val) {
    if (currentIdx < questions.length) {
        scores[questions[currentIdx].type] += (5 - val);
        currentIdx++;
        updateUI();
    }
}

function updateUI() {
    if (currentIdx < questions.length) {
        document.getElementById('question-text').innerText = questions[currentIdx].q;
        document.getElementById('progress').style.width = (currentIdx / questions.length * 100) + "%";
    } else {
        document.getElementById('quiz-card').classList.add('hidden');
        document.getElementById('result-card').classList.remove('hidden');
        const winner = Object.keys(scores).reduce((a, b) => scores[a] > scores[b] ? a : b);
        document.getElementById('result-title').innerText = results[winner].title;
        document.getElementById('result-desc').innerHTML = `
            <p class="res-p">${results[winner].desc}</p>
            <div class="fact-box">${results[winner].fact}</div>
        `;
    }
}
updateUI();
